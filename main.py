...
if __name__ == "__main__":
    params = {
        ...
        "num_task": 5,  # Number of distinct meta-tasks sampled per epoch. MetaSampler creates num_task separate environment instances
        "rollout_per_task": 20,  # Number of trajectories (rollouts) collected per task. Total trajectories per epoch = num_task * rollout_per_task
        ...
        "alpha": 0.002,  # Inner-loop learning rate (α): used for task-specific gradient descent during inner_loop adaptation
        ...
        "num_inner_grad": 1,  # Number of inner gradient descent steps per task during inner_loop (inner adaptation steps)
        ...
        # ===== Policy Optimization =====
        "clip_eps": 0.3,  # PPO clipping epsilon: controls the clipping range in PPO loss (used only by ProMP, not VPG_MAML). Inner loop: -(ratio * A).mean() without clipping
        ...
        "epochs": 500,  # Total number of meta-training epochs (outer loop iterations with new task sampling). Change to 500 for full training
        ...
        "device": torch.device("cpu")  # Computation device: torch.device("cpu") for CPU, torch.device("cuda") for GPU, torch.device("cuda:0") for specific GPU
    }
    main(params)
